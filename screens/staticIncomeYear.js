...
const statisExpenseMonth = ({ navigation }) => {
  ...
  const handleMonthChange = async (newDate) => {
    ...
    setYear(new_year);
    setCurrTime('Năm ' + new_year.toString());
  };
  ...
  const fetchData = async () => {
    try {
      ...
      setYear(currYear);
      setCurrTime('Năm ' + currYear.toString());
    } catch (error) {
  ...
  useFocusEffect(
    useCallback(async () => {
      ...
      return () => {
        console.log('Thống kê chi tiêu năm is unfocused');
      };
    }, [])
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.container_2}>
        <View>
          ...
          <Text style={{ color: '#00E879', fontSize: 13, fontWeight: 'bold' }}>
            +{totalExpense} VND
          </Text>
        </View>
        <View>
